...
function createPawn(size, color) {
    ...
    var body = new THREE.Mesh(new THREE.SphereGeometry(bodyRadius, 32, 16), pieceMaterial[color]);
    body.position.y = baseHeight + bodyRadius;

    var neckHeight = size;
    var neckRadius = bodyRadius * 0.7;
    var neck = new THREE.Mesh(new THREE.CylinderGeometry(0, neckRadius, neckHeight, 32, 1), pieceMaterial[color]);
    neck.position.y = baseHeight + bodyRadius * 2 + neckHeight / 2;

    var headRadius = size * 0.2;
    var head = new THREE.Mesh(new THREE.SphereGeometry(headRadius, 32, 16), pieceMaterial[color]);
...
function createRook(size, color) {
    ...
    var body = new THREE.Mesh(
        new THREE.CylinderGeometry(bodyRadius * 1.2, bodyRadius, bodyHeight, 32, 16),
        pieceMaterial[color]
    );
    ...
    var wedge = new THREE.Mesh(
        new THREE.CylinderGeometry(wedgeRadius, bodyRadius * 1.2, wedgeHeight, 32, 1),
        pieceMaterial[color]
    );
    ...
    var teethGeo = new THREE.TubeGeometry(
        otr,
        obr,
        itr,
        ibr,
...
function createKnight(size, color) {
    ...
    var _neck = new THREE.Mesh(
        new THREE.CylinderGeometry(neckTopRadius, neckBottomRadius, neckHeight, 32, 1),
        pieceMaterial[color]
    );
    ...
    var skull = new THREE.Mesh(new THREE.SphereGeometry(skullRadius, 32, 16), pieceMaterial[color]);
    F;
    ...
    var face = new THREE.Mesh(
        new THREE.CylinderGeometry(faceTopRadius, faceBottomRadius, faceHeight, 32, 1),
        pieceMaterial[color]
    );
    ...
    var nose = new THREE.Mesh(new THREE.SphereGeometry(noseRadius, 32, 16), pieceMaterial[color]);
...
function createQueen(size, color) {
    ...
    var collar = new THREE.Mesh(
        new THREE.CylinderGeometry(collarRadius, collarRadius, collarHeight, 32, 1),
        pieceMaterial[color]
    );
...
function createKing(size, color) {
    ...
    var collar = new THREE.Mesh(
        new THREE.CylinderGeometry(collarTopRadius, collarBottomRadius, collarHeight, 32, 1),
        pieceMaterial[color]
    );
    collar.position.y = baseHeight + bodyRadius * 2 + neckHeight - size * 0.1;
